...
const main = async () => {
  p.intro('Create a new Sniper integration block')
...
const createTsConfig = async (path: string) => {
  writeFileSync(
    ...
    await prettier.format(
      JSON.stringify({
        extends: '@sniper.io/tsconfig/base.json',
        include: ['**/*.ts', '**/*.tsx'],
...
async function addBlockToRepoDefinitions(
  ...
  const importStatement = `import { ${camelCaseId}Block } from '@sniper.io/${id}-block'`
...
async function addBlockToRepoSchemas(
  ...
  const importStatement = `import { ${camelCaseId}Block } from '@sniper.io/${id}-block'
import { ${camelCaseId}BlockSchema } from '@sniper.io/${id}-block/schemas'`
...
async function addBlockToCredentialsRepoSchemas(
  ...
  const importStatement = `import { ${camelCaseId}Block } from '@sniper.io/${id}-block'
import { ${camelCaseId}CredentialsSchema } from '@sniper.io/${id}-block/schemas'`
...
async function addBlockToRepoPackageJson(id: string) {
  ...
  packageJson.devDependencies[`@sniper.io/${id}-block`] = 'workspace:*'
  writeFileSync(
