...
It works well for teams with developers who prefer direct integration and predictable costs. **Meta only bills for conversation fees, which keeps pricing simple.**
...
### Pricing structure and conversation fees

With the Cloud API, [pricing](./whatsapp-business-api-pricing.mdx) is based on conversations.

That means there is no monthly fee to Meta, just pay for each 24-hour conversation, which is grouped by category: marketing, utility, service, or authentication. Prices also depend on the user’s region.

**Going direct with the Cloud API is usually the cheapest option.** Third-party BSPs may add their own markups to cover hosting and support tools, but you skip infrastructure maintenance costs and only pay the actual conversation fees.
...
**The transparent cost setup stands out, because you pay Meta’s official conversation fees directly.** 360dialog doesn’t tack anything extra onto those.
...
Typebot serves as the **chatbot brain for your WhatsApp Business API setup**. Providers like 360dialog or Twilio handle the connection piece. Typebot lets you control and design the conversation itself, thanks to its no-code builder.
...
You can self-host the platform for free, paying only your own server costs and Meta's conversation fees. If you would rather not manage hosting yourself, Typebot Cloud is available:
...
It’s pay-as-you-go, with no monthly platform fees for core API access. You pay a small charge per message, about $0.005, plus whatever Meta charges for regional conversations.
...
Meta’s conversation fees are handled separately.
...
Interakt’s pricing is designed for SMB ecommerce, and Meta’s conversation fees are billed separately.
...
Meta’s usual conversation fees apply for outbound campaigns.
...
Besides Meta’s required conversation fees, there are a few other potential costs to watch for:
