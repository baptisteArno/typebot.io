...
Action execution logic is defined in a separate `handlers.ts` file using the `createHandler()` function. This separates the action definition (schema, options) from its runtime implementation.
...
Each handler connects an action definition to its implementation using `createHandler(action, implementation)`.
