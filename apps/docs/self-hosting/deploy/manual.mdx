...
2. Make sure you have `Bun` CLI installed: https://bun.sh/docs/installation

3. Setup environment variables by copying the example files and following the [configuration guide](/self-hosting/configuration) to fill in the missing values.
...
<Note>
  ...
  the database with the `bun prisma generate && bun db:migrate` command.
</Note>
...
1. Cd into the builder directory and make sure it can be started with `bun start`
...
1. Cd into the viewer directory and make sure it can be started with `bun start`
