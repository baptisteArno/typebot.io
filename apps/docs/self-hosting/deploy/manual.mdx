...
<Warning>
  This doc is for deploying Sniper on a server manually. If you're looking for
  running Sniper locally, for development purposes, check out the [local
...
```sh
git clone git@github.com:<username>/sniper.io.git
cd sniper.io
git checkout latest
```
...
```sh
pm2 start --name=sniper pnpm -- start
# or select a different port
pm2 start --name=sniper pnpm -- start -p 3001
```
...
pm2 start --name=sniper pnpm -- start
# or select a different port
pm2 start --name=sniper pnpm -- start -p 3002
```

## Nginx configuration

You can use the following configuration to serve the builder and viewer with Nginx. Make sure to replace the `server_name` with the respective domain name for your Sniper instance. Check out [this guide](https://www.coderrocketfuel.com/article/how-to-deploy-a-next-js-website-to-a-digital-ocean-server/) for a step-by-step guide on how to setup Nginx and PM2.

```nginx
server {
     listen 80;
     server_name sniper.example.com www.sniper.example.com;
     return 301 https://sniper.example.com$request_uri;
}
