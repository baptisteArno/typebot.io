...
| NEXTAUTH_URL_INTERNAL                                             |         | The internal builder base URL. You have to set it only when `NEXTAUTH_URL` can't be reached by your builder container / server. For a docker deployment, you should set it to `http://localhost:3002`.        |
...
3. Create a OAuth client ID. This will be your `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`
   ...
   - For development:
     ...
     - http://localhost:3002/api/credentials/google-sheets/callback
