...
| WEBHOOK_RELAY_SECRET | | Required for Webhook blocks, including previews and WhatsApp. Use the same server-only secret in builder, viewer and PartyKit, generated from at least 32 random bytes. See the [upgrade instructions](./breaking-changes#webhook-signing-secret-and-coordinated-upgrade). |
