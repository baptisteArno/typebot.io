...
For previews, the first URL in `NEXT_PUBLIC_VIEWER_URL` must use a different hostname from `NEXTAUTH_URL`. A different port is not enough because browser cookies ignore ports. For example, locally use `NEXTAUTH_URL=http://localhost:3000` and `NEXT_PUBLIC_VIEWER_URL=http://127.0.0.1:3001`.
...
## SMTP configuration tests in the builder

The Send Email block's SMTP configuration test permits public SMTP destinations by default. To permit an internal IPv4 SMTP relay, add its exact hostname to `SSRF_ALLOWED_HOSTS` (comma-separated), for example `SSRF_ALLOWED_HOSTS=smtp.internal.example`. This existing setting only permits RFC1918 ranges (10/8, 172.16/12 and 192.168/16, including IPv4-mapped addresses). It never permits metadata, loopback, link-local, IPv6 unique-local or other blocked special-use destinations, even in development. This instance-wide exception makes that relay available to every authenticated user who can test an SMTP configuration; it is not a workspace permission.

Every DNS answer must pass the policy. The test connects to the first validated IP, preserving the hostname for TLS certificate verification and STARTTLS. Automatic fallback to another IP is disabled; if the first address is unreachable, the test fails. Use an ASCII/punycode hostname or an IP literal and an integer port between 1 and 65535. The notification/authentication SMTP settings above and execution of stored Send Email credentials are separate paths and are unchanged by this test endpoint policy.
...
| WHATSAPP_PREVIEW_APP_SECRET           |                            | The Meta app secret used to verify preview webhook signatures                                                                                |
...
| WEBHOOK_RELAY_SECRET | | Required for Webhook blocks, including previews and WhatsApp. Use the same server-only secret in builder, viewer and PartyKit, generated from at least 32 random bytes. See the [upgrade instructions](https://github.com/baptisteArno/typebot.io/blob/main/CHANGELOG.md#webhook-signing-secret-and-coordinated-upgrade). |
