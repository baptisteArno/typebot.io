...
Variables are placeholders for content that you can then use anywhere in the Sniper. It's a very important concept to understand to truly create a customized experience for the user.
...
Let's say my sniper contains these variables:
...
`https://sniper.io/my-bot?Email=test@test.com&First%20name=John` (Note that spaces in variable names should be replaced by `%20`)
