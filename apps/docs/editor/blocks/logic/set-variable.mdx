...
This will set your variable with either `desktop`, `tablet` or `mobile` depending on the device type. Under the hood, it uses the `window.matchMedia` API to detect the screen size. We don't try to really detect the device type but rather infer it from the screen size. We use this simple function:
