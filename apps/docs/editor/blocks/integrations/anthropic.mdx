...
With the Anthropic block, you can create chat messages based on your user queries and display the answer back to your sniper using Claude AI.
...
Finally, save the response of the assistant to a variable in order to append it in the chat history and also display it on your sniper.
...
`Create Chat Message` and `Ask Assistant` blocks support vision. This means that Sniper automatically detects images URL in any user message provided to OpenAI and parse it. The URL needs to be isolated from the rest of the text message to be properly detected. Here is an example of a message with an image URL:
