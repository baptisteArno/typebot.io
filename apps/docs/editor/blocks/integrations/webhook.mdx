...
This video provides a step-by-step guide to successfully configure HTTP request blocks in Sniper.
...
What I need in my case is instead of inserting "Star Wars", I'd like to insert a Sniper variable:
...
Feel free to ask the [community](https://sniper.io/discord) for help if you struggle setting up a Webhook block.
...
That is expected, Sniper actually computes all the blocks between each input blocks from a server before displaying anything.
