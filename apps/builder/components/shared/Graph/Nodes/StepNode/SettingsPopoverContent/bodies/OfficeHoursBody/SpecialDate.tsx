import {
  FormArea, FormControlRow, Label, OptionRadio, Options, SpecialDateContainer,
...
import { Box, FormControl, HStack, Stack, Text } from '@chakra-ui/react'
import { ItemWithId, TableList } from 'components/shared/TableList'
import { OctaDivider } from 'components/octaComponents/OctaDivider/OctaDivider'
import { OfficeHoursIntervalTimer, getNumberWithTwoDigits } from './OfficeHoursIntervalTimer'
...
export const SpecialDateComponent = (props: TableListItemProps<ItemWithId<SpecialDate>>) => {
  const date = props?.item?.date ? new Date(props.item.date) : new Date()
  date.setTime(date.getTime() + 3*3600*1000)
  const [currentDate, setCurrentDate] = useState<Date>(date)
  const [isAvailable, setIsAvailable] = useState<boolean>((props?.item?.hours?.length || 0) > 0)
  const [intervals, setIntervals] = useState<Array<Interval>>([...(props?.item?.hours ||[{ start: undefined, end: undefined }])])
  ...
  const changeDate = (date: Date) => {
    setCurrentDate(date)
    const dateAsString = date.getFullYear() + '-' + getNumberWithTwoDigits(date.getMonth()) + '-' + getNumberWithTwoDigits(date.getDate()) + 'T00:00:00'
  ...
  return (
    <Box borderWidth={"1.5px"} borderRadius={"5px"} width={"100%"} padding="10px">
      <Container>
