...
export const getNumberWithTwoDigits = (number: number) => number.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false })
export const OfficeHoursIntervalTimer = (props: TableListItemProps<Interval>) => {

  let hoursByQuarter: Array<OptionType> = []
  let control = 0
  for (let i = 0; i < 24; i++) {
    hoursByQuarter = [...hoursByQuarter, ...[0, 15, 30, 45].map(curr => {
      const label = getNumberWithTwoDigits(i) + ':' + getNumberWithTwoDigits(curr)
      return { key: control++, label, value: label + ':00' }
    })]
  }

  useEffect(() => {
    const minIndex = hoursByQuarter.findIndex(h => h.value === props.item.minHour)
  ...
  useEffect(() => {
    if (!initialHour)
      setInitialHour(initialHours.find(s => s.value === props.item.start) || undefined)
  }, [initialHours])

  useEffect(() => {
    if (!endHour) 
      setEndHour(finalHours.find(s => s.value === props.item.end) || undefined)
  }, [finalHours])

  useEffect(() => {
    if (props.item.end)
      setFinalHours([...hoursByQuarter.filter(h => h.key > (initialHour?.key || 0))])
  }, [initialHour])

  const onChangeInitialHour = (newValue: SingleValue<OptionType>, metaOption: ActionMeta<OptionType>) => {
    ...
    if (value.key > (endHour?.key || 0)) {
      endToSend = undefined
      onChangeFinalHour(null, { action: 'clear' } as ActionMeta<OptionType>, true)
    }

    props.onItemChange({ ...props.item, start: value.value, end: endToSend })
  }

  const onChangeFinalHour = (newValue: SingleValue<OptionType>, metaOption: ActionMeta<OptionType>, ignoreEvent?: boolean) => {
    setEndHour(newValue as OptionType)

    if (ignoreEvent) return

    props.onItemChange({ ...props.item, start: initialHour?.value, end: newValue?.value })
  }
