...
const getKey = (
  ...
  if (pageIndex === 0) return `/embed/builder/api/typebots/${typebotId}/results?limit=50`
  return `/embed/builder/api/typebots/${typebotId}/results?lastResultId=${
...
export const deleteResults = async (typebotId: string, ids: string[]) => {
  ...
  return sendRequest({
    url: `/embed/builder/api/typebots/${typebotId}/results?${params}`,
    method: 'DELETE',
  })
}
...
export const deleteAllResults = async (typebotId: string) =>
  sendRequest({
    url: `/embed/builder/api/typebots/${typebotId}/results`,
    method: 'DELETE',
  })
...
export const getAllResults = async (typebotId: string) => {
  ...
  do {
    ...
    const { data } = await sendRequest<{ results: ResultWithAnswers[] }>({
      url: `/embed/builder/api/typebots/${typebotId}/results?${query}`,
      method: 'GET',
    })
