...
test("can edit items", async ({ page }) => {
  ...
  await page.getByTestId("block block2").getByRole("button").nth(1).hover();
  await page.getByRole("button", { name: "Add Picture Choice" }).click();
  ...
  await page.getByTestId("block block2").getByRole("button").nth(2).hover();
  await page.getByRole("button", { name: "Add Picture Choice" }).click();
  await page.getByTestId("item-node").last().click();
  await expect(page.getByRole("button", { name: "Pick an image" })).toHaveCount(
    1,
  );
  await page.getByRole("button", { name: "Pick an image" }).click();
